...
\begin{docCommand}
	...
	In order for the math functions to work, the base units must at present all have a conversion path to the right-most unit, eg. if a unit group consists of base units \texttt{A, B, C}, there must be defined factors for \texttt{A\rightarrow B} and \texttt{B\rightarrow C}. The factor \texttt{A\rightarrow C} is optional; if not configured, an attempt to calculate and cache it will be made internally.
