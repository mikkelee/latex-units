...
In order to align values in a tabular context, the \docAuxCommand*{nduHeader} macro and \docAuxKey*{aligned} key wrap each segment in a cell of equal width, using  \docAuxCommand*{makebox}.
...
\begin{docCommand}
	{nduValue}
	{\marg{unit name}\oarg{options}\marg{value}}
	See \refCom{nduValue} for possible arguments.
\end{docCommand}

\begin{dispExample*}{
	title=Example usage: \docAuxCommand*{nduHeader} and \docAuxCommand*{nduValue} macros
}
\begingroup
\begin{tabular}{r r}
	\toprule
...
\begin{docCommands}[]{
	{
		doc name = nduMath,
		doc parameter = \marg{unit name}\oarg{options}\marg{result name}\marg{operator}\marg{value}
	},
	{
		doc name = nduResult,
...
The arguments of \docAuxCommand*{nduMath} are identical to those of the \refCom{nduValue} macro, except for the addition of the \meta{result name} argument, under which the result will be accumulated. It does not expand to any output. The first time a result name is used, it is assumed that the value is 0.

The \docAuxCommand*{nduResult} macro takes the \meta{variable name} and formats it for display in the same way as \refCom{nduValue}.
...
Setting either of these keys will cause all uses of \docAuxCommand*{nduValue} in the current group to be added to or subtracted from a variable of the given name.
