...
If no special configuration is made, the number of decimal points and the values between them determine how many and which units are displayed. For example, empty segments are skipped unless the \refKey{replace nil with} key is set.
...
\begin{docKey}
	{format}
	{=\marg{...}}
	{initially \meta{\docAuxCommand*{VALUE}\docAuxCommand*{ }\docAuxCommand*{SYMBOL}}}
	...
	{
		{
			doc name = {prefix},
			doc parameter = {=\meta{...}},
			doc label = general:prefix,
			doc description = {no default, initially undefined},
		},
		{
			doc name = {suffix},
			doc parameter = {=\meta{...}},
			doc label = general:suffix,
			doc description = {no default, initially undefined},
		},
	}

	When displaying a value, the output can be wrapped between \meta{prefix} and \meta{suffix}. % TODO
...
All segments will be included in the headers and cells, whether they contain a value or not. If no value is provided for the segment, and no nil replacement is specified with the \refKey{replace nil with} key, the cell will be empty. % TODO FIX
...
\nduKeys{
	treat zero as nil,
	display=values only, % TODO auto display=values only in aligned mode?
	cell width=5em,
}
\begin{tabular}{r r}
	\toprule
	...
	\midrule
	a & \nduValue{danish rigsdaler}{1.2.3} \\
	b & \nduValue{danish rigsdaler}{100..} \\ % TODO dont skip in aligned mode!
	c & \nduValue{danish rigsdaler}{.1.} \\
	\bottomrule
\end{tabular}
\endgroup
...
Note that the \docAuxKey{treat zero as nil} key does not work for math results at present. % TODO check if it does
...
\nduHeader{danish rigsdaler} % TODO newlines????
...
\begin{docCommand}
	...
	In order for the math functions to work, the base units must at present all have a conversion factor to the right-most unit, eg. if a unit group consists of base units \texttt{A, B, C}, there must be defined factors for \texttt{A\rightarrow C} and \texttt{B\rightarrow C}. The factor \texttt{A\rightarrow B} is optional, as it is not used in the internal calculations for this unit group. % TODO implement?
